...
class AuthRepository extends AuthRepoAbstract {
  ...
  Future<bool> signUp({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Fluttertoast.showToast(msg: 'Email/Имя пользователя уже существуют');
        return false;
      } else {
        return false;
      }
    } on Exception catch (_) {
      Fluttertoast.showToast(msg: 'Ой... Что-то пошло не так... Попробуйте еще раз!');
      return false;
    }
  }
  ...
  Future<bool> sendPasswordResetLink(String email) async {
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint('Unknown error in sendPasswordResetLink');
      return false;
    }
  }
  ...
  Future<Member?> googleSignIn() async {
    ...
    final GoogleSignInClientAuthorization? authorization =
        await googleSignInAccount.authorizationClient.authorizationForScopes(['email']);
